...
data class Quaternion(
    ...
    inline fun setToFunc(callback: (Int) -> Float): Unit = setTo(callback(0), callback(1), callback(2), callback(3))

    fun setTo(x: Float, y: Float, z: Float, w: Float): Unit {
        this.x = x
        this.y = y
        this.z = z
        this.w = w
    ...
    fun setTo(euler: EulerRotation): Unit = setTo(EulerRotation.toQuaternion(euler, this))
    fun setTo(other: Quaternion): Unit = setTo(other.x, other.y, other.z, other.w)
    ...
    fun setToNlerp(left: Quaternion, right: Quaternion, t: Float): Unit {
        val sign = if (dotProduct(left, right) < 0f) -1f else +1f
        setToFunc { (1f - t) * left[it] + t * right[it] * sign }
        normalize()
    }

    fun setToInterpolated(left: Quaternion, right: Quaternion, t: Float): Unit = setToSlerp(left, right, t)
