...
class Vector3D private constructor(private val data: FloatArray) {
    ...
    constructor(x: Int, y: Int, z: Int): this(floatArrayOf(x.toFloat(), y.toFloat(), z.toFloat()))
    constructor(x: Double, y: Double, z: Double): this(floatArrayOf(x.toFloat(), y.toFloat(), z.toFloat()))
    constructor(): this(0.0f, 0.0f, 0.0f)

    fun copyFrom(other: Vector3D): Unit = setTo(other.x, other.y, other.z)
    ...
    inline fun setTo(func: (index: Int) -> Float): Unit = setTo(func(0), func(1), func(2))

    inline fun setTo(l: Vector3D, r: Vector3D, func: (l: Float, r: Float) -> Float): Unit = setTo(
        func(l.x, r.x),
        func(l.y, r.y),
        func(l.z, r.z)
    )

    fun setToInterpolated(left: Vector3D, right: Vector3D, t: Float): Unit = setTo { t.interpolate(left[it], right[it]) }

    fun scale(scale: Float): Unit = setTo(x * scale, y * scale, z * scale)
    ...
    fun setToSub(l: Vector3D, r: Vector3D): Unit = setTo(l.x - r.x, l.y - r.y, l.z - r.z)

    fun setToAdd(l: Vector3D, r: Vector3D): Unit = setTo(l.x + r.x, l.y + r.y, l.z + r.z)

    fun setToCross(l: Vector3D, r: Vector3D): Unit = setTo(
        (l.y * r.z - l.z * r.y),
        (l.z * r.x - l.x * r.z),
        (l.x * r.y - l.y * r.x)
    )
     ...
     * Cross product
     */
    operator fun timesAssign(other: Vector3D): Unit = setTo(
        (y * other.z - z * other.y),
        (z * other.x - x * other.z),
        (x * other.y - y * other.x)
    )
